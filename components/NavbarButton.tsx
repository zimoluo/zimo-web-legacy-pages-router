...
import { iconTextMap, topIconMap, ThemeType, svgFilterMap, textColorMap } from './themeMaps'; // Update the path accordingly
    ...
    <Link href={`/${item}`} passHref>
      <div className="group font-arial bg-transparent px-6 py-1 rounded relative min-w-fit font-bold h-6 cursor-pointer">
        <div className="absolute inset-0 flex items-center justify-center transition-opacity duration-300 opacity-100">
          <img src={`/${topIconMap[item]}`} className={`h-6 mr-2 transform rounded transition-all duration-300 group-hover:scale-125 ${svgFilterClass} ${shouldApplyGlow}`} alt={`${iconTextMap[item]} Icon`} />
