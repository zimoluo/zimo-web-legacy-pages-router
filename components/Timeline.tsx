...
const Timeline: React.FC<TimelineProps> = ({ events }) => {
  ...
  return (
    <div className="relative w-96 mx-0">
      <div className="absolute top-0 bottom-0 left-1/2" />
      {sortedEvents.map(([date, text], index) => (
        <div key={index} className={`relative p-4 w-1/2 left-1/3`}>
          ...
          <div
            className={`bg-neutral-50 bg-opacity-40 backdrop-blur-xl shadow-lg p-4 rounded-md ml-6 max-w-md`}
